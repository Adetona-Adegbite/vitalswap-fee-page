...
const FeeCards = ({ userType }: FeeCardsProps) => {
      ...
      {cards.map((card) => (
        ...
        >
          <div className={`bg-gradient-to-r ${card.gradient} p-6 text-white`}>
            ...
            <h3 className="text-xl font-bold mb-2" style={{ fontFamily: 'Gilroy, sans-serif' }}>
              {card.title}
            </h3>
          ...
          <div className="p-6">
            ...
            >
              See Details
              <i className={`ri-arrow-${expandedCard === card.id ? 'up' : 'down'}-s-line transition-transform duration-300`}></i>
            </button>
              ...
              <div className="mt-4 space-y-3 animate-fadeIn">
                {card.fees.map((fee, index) => (
                  <div key={index} className="flex justify-between items-center py-2 border-b border-gray-100 last:border-b-0">
                    ...
                    <span className="font-semibold text-gray-900" style={{ fontFamily: 'Gilroy, sans-serif' }}>
                      {fee.amount}
                    </span>
                  </div>
                ))}
